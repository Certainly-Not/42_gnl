...
/*   Updated: 2023/03/19 18:12:52 by elallema         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || fd > FOPEN_MAX)
		return (NULL);
	keep = NULL;
	keep = buff_to_keep(keep, buff[fd]);
	read_size = BUFFER_SIZE;
	while (read_size == BUFFER_SIZE && !has_linebreak(buff[fd]))
	{
	...
	line = keep_to_line(keep, buff[fd]);
	free(keep);
	return (line);
}
